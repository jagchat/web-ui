...
$(function () {
	...
	({
		empno,
		ename
	} = oEmp2); //paranthesis are essential
	console.log(`Id = ${empno}`);
	console.log(`Name = ${ename}`);
	...
	let {
		[empId]: id2
	} = oEmp2; //destructuring with dynamic property name in a variable
	console.log(`Id2 = ${id2}`);

	//-->a string can be destructured too
	let s = "Hello";
	let [a, , , , e] = s; //a="H", e="o"
	console.log(`a = ${a}, e = ${e}`);
	...
	let {
		designation = '-NA-'
	} = oEmp3; //no issues with this (oEmp3 has 'designation')
	console.log(`Designation = ${designation}`);

	let {
		designation: desig = '-NA-'
	} = oEmp3; //variable renaming
	console.log(`Designation = ${desig}`);

	//compile-time error
	// let {
	// 	designation = '-NA-'
	// } = oEmp2; //oEmp2 does not have 'designation' 
	// console.log(`Designation = ${designation}`);

	//can send options as last parameter as follows:
	let doProcess = function (a, b, {
		option1,
		option2,
		option3
	}) {
